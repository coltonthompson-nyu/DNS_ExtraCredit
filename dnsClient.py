...
def dns_query(type, name, server):
    ...
    server_address = (server, 53) # Enter Port Number 53
    ...
    qname_parts = name.split('.') # How can we easily split the string? # split on '.'
    # ref https://www.w3schools.com/python/ref_string_encode.asp
    ...
    if type == 'A':
        ...
        qtype = 28     # Lookup the Resource Record value #ref https://en.wikipedia.org/wiki/List_of_DNS_record_types
    else:
    ...
    response_header = data[:12] # What is the size of the DNS response header in bytes? - 12 bytes
